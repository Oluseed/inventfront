...
const data = [
  {
    id: 1,
    icon: icon_1,
    headingKey: "manageStore.cards.zeroDelay.heading",
    descriptionKey: "manageStore.cards.zeroDelay.description",
  },
  {
    id: 2,
    icon: icon_2,
    headingKey: "manageStore.cards.noHiddenFee.heading",
    descriptionKey: "manageStore.cards.noHiddenFee.description",
  },
  {
    id: 3,
    icon: icon_3,
    headingKey: "manageStore.cards.alerts.heading",
    descriptionKey: "manageStore.cards.alerts.description",
  },
];
        ...
        >
          <h1 className="mt-10 font-clashDisplay font-medium text-3xl text-center sm:text-4xl md:text-5xl">
            <span className="border-[#FF4C05] border-2 rounded-4xl py-1 px-2">
              {t("manageStore.title.highlight")}
            </span>{" "}
            {t("manageStore.title.rest")}
          </h1>
        </motion.div>
            ...
            <div className="mx-5 flex flex-row gap-x-3">
              {data.map((item) => (
                <motion.div
                  ...
                  transition={{ duration: 0.8, ease: "easeOut" }}
                >
                  <img src={item.icon} alt={t(`${item.headingKey}`)} className="w-10 h-10" />
                  <h1 className="font-clashDisplay font-medium text-[#000000] text-[18px]">
                    {t(item.headingKey)}
                  </h1>
                  <p className="font-inter text-[#5E5E5E] text-[12px]">
