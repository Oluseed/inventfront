      ...
      >
        <div className='text-center lg:ml-5'>
          ...
          <motion.div className="md:mt-10 flex flex-wrap items-center justify-center px-4" variants={fadeInUp}>
            <motion.img src={icon_1} alt="icon-1" className="w-10 mr-2 sm:mr-10 h-10 sm:w-15 sm:h-15 md:w-20 md:h-20"
              whileHover={{ rotate: 15, scale: 1.1 }} transition={{ type: "spring", stiffness: 200 }} />
            <h1 className="font-clashDisplay text-[#070707] text-2xl sm:text-3xl md:text-5xl font-medium text-center">
              {t('pricingHero.row1.title1')}
            </h1>

            <h1 className="ml-1 font-clashDisplay text-[#070707] text-2xl sm:text-3xl md:text-5xl font-medium text-center">
              {t('pricingHero.row1.and')}
            </h1>
            <motion.img src={icon_2} alt="icon-2" className="w-6 sm:mr-10 h-6 sm:w-8 sm:h-8 md:w-15 md:h-15"
              whileHover={{ rotate: -15, scale: 1.1 }} transition={{ type: "spring", stiffness: 200 }} />
          </motion.div>
          ...
          <motion.div className="flex flex-wrap items-center justify-center px-4 sm:ml-10 md:ml-20" variants={fadeInUp}>
            <h1 className="font-clashDisplay text-[#070707] text-2xl sm:text-3xl md:text-5xl font-medium text-center">
              {t('pricingHero.row2.title')}
            </h1>
            <motion.img src={icon_3} alt="icon-3" className="w-6 sm:ml-10 h-6 sm:w-10 sm:h-10 md:w-20 md:h-20"
          ...
          <motion.div className="flex items-start sm:mx-2 md:mx-10" variants={fadeInUp}>
            <div className="flex-shrink-0 hidden sm:block">
              <motion.img src={icon_4} alt="icon-4" className="w-6 h-6 sm:w-10 sm:h-10 md:w-20 md:h-20"
                whileHover={{ rotate: 360 }} transition={{ duration: 1 }} />
            </div>
            <p className="ml-3 sm:ml-4 text-[#5E5E5E] text-sm sm:text-xl leading-relaxed">
              {t('pricingHero.paragraph')}
            </p>
          </motion.div>

          {/* Contact Button */}
          <motion.div className="grid place-items-center gap-4" variants={fadeInUp}>
            <motion.img src={icon_5} alt="icon-5" className="w-12 h-12 sm:w-15 sm:h-15 md:w-25 md:h-25"
            ...
            <NavLink to={'/contact'}>
              <motion.button
                ...
                hover:from-[#e0440b] hover:to-[#d65f28] px-5 sm:px-6 py-2.5 sm:py-3 rounded-xl shadow-md hover:shadow-lg
                font-montserrat text-sm sm:text-base font-medium text-white transition-all duration-300 group"
              >
                <motion.img src={buttonIcon} alt="button-icon" className="w-5 h-5 transform transition-all duration-300"
                  whileHover={{ x: 5 }} />
                {t('pricingHero.contactButton')}
              </motion.button>
            </NavLink>
          </motion.div>
        </div>
